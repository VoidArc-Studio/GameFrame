...
impl OpenGLBackend {
    pub fn new(config: &Config) -> Self {
        ...
        let surface = unsafe {
            display
                .create_window_surface(&config, &window)
        ...
        let context_attributes = ContextAttributesBuilder::new()
            .with_context_api(glutin::context::ContextApi::OpenGl(Some(
                match config.gpu.opengl_version.as_str() {
                    "2.1" => glutin::context::Version {
                        major: 2,
                        minor: 1,
                    },
                    _ => glutin::context::Version {
                        major: 4,
                        minor: 5,
                    }, // Default to modern OpenGL
                },
            )))
            .build(Some(&window));
        let context = unsafe {
        ...
        if config.rendering.vsync {
            surface.set_swap_interval(&context, glutin::surface::SwapInterval::Wait(std::num::NonZeroU32::new(1).unwrap())).expect("Failed to set vsync");
        } else {
            surface.set_swap_interval(&context, glutin::surface::SwapInterval::DontWait).expect("Failed to disable vsync");
        }
...
impl RenderBackend for OpenGLBackend {
    fn render(&mut self) {
        // Basic OpenGL rendering (placeholder)
        unsafe {
        ...
        println!(
            "Rendering with OpenGL (Vendor: {}, Resolution: {}, VSync: {})",
...
impl Drop for OpenGLBackend {
    fn drop(&mut self) {
        // Cleanup handled automatically by glutin/winit
    }
}

// Helper function to parse resolution string (e.g., "1920x1080")
